...
import com.grepp.smartwatcha.infra.neo4j.node.TagRelationship;
...
public class RecommendPersonalRatedNeo4jService {
    ...
    @Transactional("neo4jTransactionManager")
    public List<String> getTagsByMovieId(Long movieId) {
        return genreRepo.findById(movieId)
                .map(MovieNode::getTags)
                .orElse(List.of())
                .stream()
                .map(rel -> rel.getTagNode().getName())
                .toList();
    }
}